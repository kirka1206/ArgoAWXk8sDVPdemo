...
10. После исправления Victor portal убедиться, что алерт
    `IngressResponses5xx` закрылся после очистки окна метрики, и провести
    демонстрационный переход по всем пяти представлениям.
11. Выполнить `scenarios/11-practicum-end-to-end.md`, не изменяя опубликованные
   golden image v1/v2.
12. Заменить временный `SuperAdmin` пользователя `practicum-tks@demo.local` на
...
4. Доработать Python-сервис обработки заявок: принимать корректный YAML и JSON
   для `EnvironmentRequest`, либо переименовать контракт и расширение файлов
   в `.json`; после этого обновить Git-сценарий.
5. Добавить защиту ветки `main` и обязательное review platform team.
6. Исключить status-коммиты, если semantic status не изменился.
